...
analyses <- list(
  list(
    ...
    output_dir = "SBS3/SBS3_OS_from_sample_results", prefix = "SBS3",
    inset_xmin = 27, inset_xmax = 47, legend_x = 0.05, legend_y = 0.05
  ),
  list(
    ...
    output_dir = "SBS3_ID6/SBS3_ID6_OS_from_sample_results", prefix = "SBS3_ID6",
    inset_xmin = 23, inset_xmax = 47, legend_x = 0.05, legend_y = 0.05
  ),
  list(
    ...
    output_dir = "SBS6_15_44/SBS6_15_44_OS_from_sample_results", prefix = "SBS6_SBS15_SBS44",
    inset_xmin = 20, inset_xmax = 49, legend_x = 0.05, legend_y = 0.025
  )
)
...
sheet_names <- excel_sheets(syng_metadata)

if ("Patients" %in% sheet_names) {
...
patients <- read_excel(syng_metadata, sheet = patient_sheet) %>%
  ...
  select(patient_id, last_FU_OS_days, OS_EVENT) %>%
...
run_os_analysis <- function(config) {
  ...
  signature_data <- read_csv(config$file, show_col_types = FALSE) %>%
    mutate(patient_id = trimws(as.character(patient_id)), sampling_day = as.numeric(sampling_day)) %>%
  ...
  # Survival from sample collection
  
  df <- signature_data %>%
    inner_join(patients, by = "patient_id", relationship = "one-to-one") %>%
    mutate(
      OS_DAYS_FROM_SAMPLE = last_FU_OS_days - sampling_day,
      OS_MONTHS_FROM_SAMPLE = OS_DAYS_FROM_SAMPLE / 30.4375,
      OS_VALID = !is.na(OS_MONTHS_FROM_SAMPLE) & OS_MONTHS_FROM_SAMPLE > 0 &
        !is.na(OS_EVENT) & !is.na(sampling_day) & last_FU_OS_days >= sampling_day
    )
  ...
  os_df <- df %>%
    ...
    mutate(
      ...
      KM_GROUP = GROUP,
      original_time = OS_MONTHS_FROM_SAMPLE,
  ...
  km <- ggsurvplot(
    ...
    risk.table = TRUE, risk.table.height = 0.1,
    censor = TRUE, censor.shape = 3, censor.size = 2.2,
  ...
  write.csv(os_df, file.path(config$output_dir, paste0(config$prefix, "_OS_from_sample_final_dataset.csv")), row.names = FALSE)
